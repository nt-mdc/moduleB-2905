<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <a href="{{route('product.new')}}" class="mb-4 inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">New Product</a>
                ...
                <div class="p-6 text-gray-900 grid grid-cols-4 gap-4">
                    @foreach ($prods as $item)
                        <a href="{{ route('product.edit', ['gtin' => $item['gtin']]) }}">
                            ...
                            <p class="line-clamp-2">{{ $item['translations'][0]['description'] }}</p>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
            <form method="POST" action="{{ route('company') }}" class="grid grid-cols-3 gap-4">
                @csrf
